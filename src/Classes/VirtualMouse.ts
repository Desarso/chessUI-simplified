...
export class VirtualMouse {
  ...
  constructor(ws: Accessor<WebSocket>, self: User, opponent: User, board: Accessor<Board>) {
    this.ws = ws;
  ...
  private sendMouseEvents() {
    ...
    document.addEventListener("pointermove", (event) => {
      if(!this.board().inLastMove) return;
  ...
  private setVirtualPosition(data: any) {
    ...
    let destinationChessboardWidth =
      document.querySelector(".chessBoard").offsetWidth;
    ...
    oponentsCursor.style.transform = `translate(${
      invertedCursorX - this.cursorWidth / 2
    }px, ${invertedCursorY - this.cursorWidth / 2}px)`;
